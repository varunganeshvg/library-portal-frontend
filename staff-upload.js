...
async function loadBooks() {
  ...
  `http://localhost:8081/api/staff/books/class?course=${course}&section=${section}&semester=${semester}`,
  {
    headers: {
...
async function uploadBook() {
    ...
    let url = "http://localhost:8081/api/books/add";
    let method = "POST";

    if (editingBookId) {
        url = `http://localhost:8081/api/books/update/${editingBookId}`;
        method = "PUT";
    }
...
async function deleteBook(id) {
    ...
    await fetch(
        `http://localhost:8081/api/books/delete/${id}`,
        {
            method: "DELETE",
